...
#ifdef GAMECUBE
#include "DataFiles.h"
#define BACK_LOG(fmt, ...) printf("[Back] " fmt "\n", ##__VA_ARGS__)
#else
#define BACK_LOG(fmt, ...) ((void)0)
#endif
...
BOOL InitBack(const char *fName, int type)
{
	...
	if (embedded != NULL)
	{
		...
		{
			BACK_LOG("  -> ERROR: Not a BMP file!");
			return FALSE;
		}

		// Get bitmap width and height (at offset 18, little-endian)
		gBack.partsW = data[18] | (data[19] << 8) | (data[20] << 16) | (data[21] << 24);
		gBack.partsH = data[22] | (data[23] << 8) | (data[24] << 16) | (data[25] << 24);
		BACK_LOG("  -> Size: %dx%d", gBack.partsW, gBack.partsH);
	}
	else
	{
