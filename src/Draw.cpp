...
#ifdef GAMECUBE
	BOOL hires;		// Hi-res text surface (2x scale for 640x480 text quality)
#endif
} surface_metadata[SURFACE_ID_MAX];

#ifdef GAMECUBE
// Check if a surface ID is a text surface that should be hi-res
static BOOL IsTextLineSurface(SurfaceID surf_no)
{
	return (surf_no >= SURFACE_ID_TEXT_LINE1 && surf_no <= SURFACE_ID_TEXT_LINE5) ||
	       (surf_no == SURFACE_ID_CREDIT_CAST) ||   // Ending.cpp credits text
	       (surf_no == SURFACE_ID_ROOM_NAME);       // MapName.cpp room name text
}
#endif
...
#ifdef GAMECUBE
	// Use mag=1 for 320x240 coordinates - GX hardware scales to 640x480
	(void)lMagnification;
	mag = 1;
	fullscreen = TRUE;
#else
...
#ifdef GAMECUBE
	// For text line surfaces, create at 2x resolution for sharp 640x480 text
	int scale = 1;
	BOOL hires = FALSE;
	if (IsTextLineSurface(surf_no))
	{
		scale = 2;
		hires = TRUE;
		printf("[Draw] Creating hi-res text surface %d: %dx%d -> %dx%d\n", 
			surf_no, bxsize, bysize, bxsize * scale, bysize * scale);
	}
	surf[surf_no] = RenderBackend_CreateSurface(bxsize * scale, bysize * scale, bTarget);
#else
...
#ifdef GAMECUBE
	// Mark surface as hi-res in the rendering backend
	if (hires)
		RenderBackend_SetSurfaceHiRes(surf[surf_no], true);
#endif
...
#ifdef GAMECUBE
	surface_metadata[surf_no].hires = hires;
#endif
...
#ifdef GAMECUBE
	// Hi-res text surfaces are 2x size - scale source rect accordingly
	if (surface_metadata[surf_no].hires)
	{
		rcWork.left *= 2;
		rcWork.top *= 2;
		rcWork.right *= 2;
...
#ifdef GAMECUBE
	// When copying to hi-res surfaces, scale destination coordinates by 2x
	// The backend will also scale the source content 2x to match
	if (surface_metadata[to].hires)
	{
		RenderBackend_Blit(surf[from], &rcWork, surf[to], x * 2, y * 2, TRUE);
		return;
	}
#endif
...
void CortBox2(const RECT *rect, unsigned long col, SurfaceID surf_no)
{
	...
	static RenderBackend_Rect rcSet;	// TODO - Not the original variable name
#ifdef GAMECUBE
	// Hi-res text surfaces use 2x coordinates
	if (surface_metadata[surf_no].hires)
	{
		rcSet.left = rect->left * 2;
		rcSet.top = rect->top * 2;
		rcSet.right = rect->right * 2;
		rcSet.bottom = rect->bottom * 2;
	}
	else
#endif
	{
...
#ifdef GAMECUBE
	// Use 10x20 font for 640x480 quality text rendering
...
#ifdef GAMECUBE
	// Hi-res text surfaces use 2x coordinates for full resolution text
	if (surface_metadata[surf_no].hires)
	{
		DrawText(font, surf[surf_no], x * 2, y * 2, color, text);
		return;
	}
#endif
