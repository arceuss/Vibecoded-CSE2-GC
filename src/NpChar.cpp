...
#ifdef GAMECUBE
#include "DataFiles.h"
#define NPCHAR_LOG(fmt, ...) printf("[NpChar] " fmt "\n", ##__VA_ARGS__)
#else
#define NPCHAR_LOG(fmt, ...) ((void)0)
#endif
...
// Helper to read little-endian 16-bit from memory
static unsigned short NpChar_ReadLE16(const unsigned char *data)
{
	return data[0] | (data[1] << 8);
}

// Helper to read little-endian 32-bit from memory
static unsigned long NpChar_ReadLE32(const unsigned char *data)
{
...
BOOL LoadEvent(const char *path_event)
{
	...
	if (embedded != NULL)
	{
		...
		// Read "PXE" check
		if (memcmp(data, gPassPixEve, 3) != 0)
		{
			NPCHAR_LOG("  -> ERROR: Not a PXE file!");
			return FALSE;
		}
		data += 4; // Skip "PXE" + version

		// Get amount of NPCs
		count = NpChar_ReadLE32(data);
		data += 4;
		NPCHAR_LOG("  -> Loading %d NPCs", count);
		...
		for (i = 0; i < count; ++i)
		{
			// Get data from memory (each event is 12 bytes)
			eve.x = NpChar_ReadLE16(data); data += 2;
			eve.y = NpChar_ReadLE16(data); data += 2;
			eve.code_flag = NpChar_ReadLE16(data); data += 2;
			eve.code_event = NpChar_ReadLE16(data); data += 2;
			eve.code_char = NpChar_ReadLE16(data); data += 2;
			eve.bits = NpChar_ReadLE16(data); data += 2;
