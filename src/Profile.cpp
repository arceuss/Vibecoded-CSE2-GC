...
BOOL IsProfile(void)
{
	...
	{
#ifdef GAMECUBE
		Backend_PrintInfo("No existing save file found");
#endif
		return FALSE;
	}
	...
	fclose(file);
#ifdef GAMECUBE
	Backend_PrintInfo("Save file found!");
#endif
	return TRUE;
}
...
BOOL SaveProfile(const char *name)
{
	...
	if (!Backend_IsSDMounted())
	{
		Backend_PrintError("Cannot save: SD card not mounted");
		Backend_ShowMessageBox("Save Error", "SD card not available. Please insert SD Gecko.");
		return FALSE;
	}
#endif
